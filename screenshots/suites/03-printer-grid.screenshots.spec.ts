...
test.describe('Printer Grid Screenshots', () => {
  ...
  test('01-printer-grid-empty', async ({ authenticatedPage, apiMock }) => {
    ...
    await authenticatedPage
      .waitForSelector('[data-testid="printer-grid"], .printer-grid, main', {
        timeout: 5000
      })
      .catch(() => {})
    ...
    await captureFullPage(
      authenticatedPage,
      'printer-grid-empty.png',
      'printer-grid'
    )
  })

  test('02-printer-grid-with-printers', async ({
    authenticatedPage,
    apiMock
  }) => {
    ...
    await captureFullPage(
      authenticatedPage,
      'printer-grid-with-printers.png',
      'printer-grid'
    )
  })
  ...
  test('03-create-floor-dialog', async ({ authenticatedPage, apiMock }) => {
    ...
    const createFloorBtn = authenticatedPage
      .locator('button')
      .filter({ hasText: /create.*first.*floor/i })
      .first()
  ...
  test('04-create-printer-dialog', async ({ authenticatedPage, apiMock }) => {
    ...
    const createPrinterBtn = authenticatedPage
      .locator('button')
      .filter({ hasText: /create.*printer/i })
      .first()
  ...
  test('06-batch-operations-bar', async ({ authenticatedPage, apiMock }) => {
    ...
    const printerCards = authenticatedPage.locator(
      '.printer-card, [data-testid="printer-card"]'
    )
    ...
    await captureFullPage(
      authenticatedPage,
      'batch-operations-bar.png',
      'printer-grid'
    )
  })

  test('07-printer-grid-context-menu', async ({
    authenticatedPage,
    apiMock
  }) => {
    await apiMock.mockAllEndpoints({ loginRequired: false, emptyData: false })
