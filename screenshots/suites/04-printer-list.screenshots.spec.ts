...
test.describe('Printer List Screenshots', () => {
  ...
  test('02-printer-list-empty', async ({ authenticatedPage, apiMock }) => {
    ...
    await authenticatedPage
      .waitForSelector('[data-testid="empty-state"], .empty-state, main', {
        timeout: 5000
      })
      .catch(() => {})
  ...
  test('04-create-printer-dialog-filled', async ({ authenticatedPage }) => {
    ...
    await captureFullPage(
      authenticatedPage,
      'create-printer-dialog-filled.png',
      'printer-list'
    )
  })
  ...
  test('05-printer-details-panel', async ({ authenticatedPage }) => {
    ...
    const printerRow = authenticatedPage
      .locator('tr, .v-list-item, .printer-item')
      .filter({ hasText: /prusa|ender|artillery/i })
      .first()
  ...
  test('06-attach-floor-to-printer', async ({ authenticatedPage }) => {
    ...
    const floorSelect = authenticatedPage
      .locator('select, .v-select, [data-testid="floor-select"]')
      .filter({ hasText: /floor/i })
      .first()
  ...
  test('07-attach-camera-to-printer', async ({ authenticatedPage }) => {
    ...
    const cameraBtn = authenticatedPage
      .locator('button, .v-select')
  ...
  test('08-attach-tag-to-printer', async ({ authenticatedPage }) => {
    ...
    const tagSelect = authenticatedPage
      .locator('button, .v-select, [data-testid="tag-select"]')
