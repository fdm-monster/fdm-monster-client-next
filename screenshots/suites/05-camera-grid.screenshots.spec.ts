...
test.describe('Camera Grid Screenshots', () => {
  ...
  test('01-camera-grid-view', async ({ authenticatedPage }) => {
    ...
    await authenticatedPage
      .waitForSelector('[data-testid="camera-grid"], .camera-grid, main', {
        timeout: 5000
      })
      .catch(() => {})
  ...
  test('04-create-camera-dialog-filled', async ({ authenticatedPage }) => {
    ...
    // Fill form fields
    const inputs = authenticatedPage.locator(
      '.v-dialog input[type="text"], .v-dialog input[type="url"]'
    )
    ...
    await captureFullPage(
      authenticatedPage,
      'create-camera-dialog-filled.png',
      'cameras'
    )
  })
  ...
  test('05-camera-card-with-stream', async ({ authenticatedPage }) => {
    ...
    const cameraCard = authenticatedPage
      .locator('.camera-card, [data-testid="camera-card"]')
      .first()
