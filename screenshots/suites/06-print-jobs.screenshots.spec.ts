...
test.describe('Print Jobs Screenshots', () => {
  ...
  test('03-job-details-dialog', async ({ authenticatedPage }) => {
    ...
    const jobRow = authenticatedPage
      .locator('tr, .v-list-item, .job-item')
      .first()
  ...
  test('04-job-details-with-statistics', async ({ authenticatedPage }) => {
    ...
    const completedJob = authenticatedPage
      .locator('tr, .job-item')
    ...
    await captureFullPage(
      authenticatedPage,
      'job-details-with-statistics.png',
      'jobs'
    )
  })
  ...
  test('06-jobs-by-status-completed', async ({ authenticatedPage }) => {
    ...
    if (await statusFilter.isVisible().catch(() => false)) {
      ...
      const completedOption = authenticatedPage
        .locator('.v-list-item')
        .filter({ hasText: /completed/i })
        .first()
  ...
  test('07-jobs-by-status-failed', async ({ authenticatedPage }) => {
    ...
    if (await statusFilter.isVisible().catch(() => false)) {
      ...
      const failedOption = authenticatedPage
        .locator('.v-list-item')
