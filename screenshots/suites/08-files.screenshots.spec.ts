...
test.describe('Files Screenshots', () => {
  ...
  test('01-files-browser', async ({ authenticatedPage }) => {
    ...
    await authenticatedPage
      .waitForSelector('[data-testid="files-browser"], .files-browser, main', {
        timeout: 5000
      })
      .catch(() => {})
  ...
  test('04-file-context-menu', async ({ authenticatedPage }) => {
    ...
    const fileItem = authenticatedPage
      .locator('.file-item, [data-testid="file-item"], tr')
      .filter({ hasText: /\.gcode|\.stl/i })
      .first()
  ...
  test('06-files-grid-view', async ({ authenticatedPage }) => {
    ...
    const gridViewBtn = authenticatedPage
      .locator('button[aria-label*="grid" i], .view-toggle')
      .first()
  ...
  test('07-files-list-view', async ({ authenticatedPage }) => {
    ...
    const listViewBtn = authenticatedPage
      .locator('button[aria-label*="list" i], .view-toggle')
      .first()
