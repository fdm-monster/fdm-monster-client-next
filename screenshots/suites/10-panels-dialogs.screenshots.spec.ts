...
test.describe('Panels and Dialogs Screenshots', () => {
  ...
  test('01-yaml-export-panel', async ({ authenticatedPage }) => {
    ...
    const exportBtn = authenticatedPage
      .locator('button')
      .filter({ hasText: /yaml.*export|export.*yaml/i })
      .first()
  ...
  test('02-yaml-import-panel', async ({ authenticatedPage }) => {
    ...
    const importBtn = authenticatedPage
      .locator('button')
      .filter({ hasText: /yaml.*import|import.*yaml/i })
      .first()
  ...
  test('03-octofarm-import-panel', async ({ authenticatedPage }) => {
    ...
    const importBtn = authenticatedPage
      .locator('button')
      .filter({ hasText: /octofarm.*import|import.*octofarm/i })
      .first()
    if (await importBtn.isVisible().catch(() => false)) {
      await importBtn.click()
      await authenticatedPage.waitForTimeout(1000)
  ...
  test('04-jobs-menu', async ({ authenticatedPage }) => {
    ...
    // Open jobs menu
    const menuBtn = authenticatedPage
      .locator('button[aria-label*="menu" i], .menu-btn')
      .first()
  ...
  test('05-printer-status-menu', async ({ authenticatedPage }) => {
    ...
    const printerCard = authenticatedPage
      .locator('.printer-card, [data-testid="printer-card"]')
      .first()
    if (await printerCard.isVisible().catch(() => false)) {
      // Look for menu button on printer card
      const menuBtn = printerCard
        .locator('button[aria-label*="menu" i], .menu-btn')
        .first()
      if (await menuBtn.isVisible().catch(() => false)) {
        await menuBtn.click()
  ...
  test('07-printer-dialog-update', async ({ authenticatedPage }) => {
    ...
    const editBtn = authenticatedPage
      .locator('button')
  ...
  test('08-printer-dialog-duplicate', async ({ authenticatedPage }) => {
    ...
    const duplicateBtn = authenticatedPage
      .locator('button')
  ...
  test('09-printer-test-connection-dialog', async ({ authenticatedPage }) => {
    ...
    const testBtn = authenticatedPage
      .locator('button')
      .filter({ hasText: /test.*connection|connection.*test/i })
      .first()
    ...
    await captureFullPage(
      authenticatedPage,
      'printer-test-connection-dialog.png',
      'dialogs'
    )
  })
  ...
  test('10-printer-type-dropdown', async ({ authenticatedPage }) => {
    ...
    const typeSelect = authenticatedPage
      .locator('.v-dialog select, .v-dialog .v-select')
  ...
  test('11-printer-force-save-warning', async ({ authenticatedPage }) => {
    ...
    await captureFullPage(
      authenticatedPage,
      'printer-force-save-warning.png',
      'dialogs'
    )
  })
  ...
  test('12-edit-floor-panel', async ({ authenticatedPage }) => {
    ...
    const floorItem = authenticatedPage
      .locator('.floor-item, [data-testid="floor-item"], tr')
      .first()
