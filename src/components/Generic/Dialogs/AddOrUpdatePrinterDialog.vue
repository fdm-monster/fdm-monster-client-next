          ...
          <v-col :cols="showChecksPanel ? 8 : 12">
            ...
            <v-text-field
              ...
              :label="
                ...
                formData.printerType === MoonrakerType
                  ? 'API Key (required)*'
                  : 'API Key (unsupported)'
              "
              persistent-hint
            ...
            <v-text-field
              v-if="
                formData.printerType === PrusaLinkType ||
                formData.printerType === BambuType
              "
              v-model="formData.username"
              class="ma-1"
              :hint="
                formData.printerType === BambuType
                  ? 'Serial number from printer'
                  : 'Username (often \'maker\')'
              "
              :label="
                formData.printerType === BambuType ? 'Serial' : 'Username'
              "
              persistent-hint
              required
            />
            ...
            <v-text-field
              ...
              :hint="
                formData.printerType === BambuType
                  ...
                  : 'Password (visit your printer settings)'
              "
              :label="
...
const serviceTypes = computed(() => {
  if (featureStore.hasFeature('multiplePrinterServices')) {
    ...
    const hasPrusaLinkSupport =
      feature?.subFeatures?.types?.includes('prusaLink')
...
const submitButtonText = computed(() => {
  return (
    (forceSavePrinter.value ? 'Force ' : '') +
    (isUpdating.value ? 'Save' : 'Create')
  )
})
...
async function createPrinter(newPrinterData: CreatePrinter) {
  const printer = await printersStore.createPrinter(
    newPrinterData,
...
async function submit() {
  ...
  if (
    isMoonrakerType(createdPrinter.printerType) ||
    isPrusaLinkType(createdPrinter.printerType) ||
    isBambuType(createdPrinter.printerType)
  ) {
