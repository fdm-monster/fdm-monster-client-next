<template>
  ...
  >
    ...
    <v-card elevation="1">
      <v-card-title class="d-flex align-center py-2">
        <v-icon
          ...
          color="primary"
          size="small"
          >{{ activeTab === 'jobs' ? 'list_alt' : 'queue' }}</v-icon
        >
        <span class="text-subtitle-1">{{
          activeTab === 'jobs' ? 'Results' : 'Queue'
        }}</span>
        <v-spacer />

        <div
          v-if="activeTab === 'jobs' && !loading && totalJobs > 0"
        ...
        <div
          v-if="activeTab === 'queue' && !loadingQueue && queueCount > 0"
          class="text-caption text-medium-emphasis"
        >
      ...
      <v-card-text class="pa-0">
        <v-data-table-server
          ...
          :loading-text="
            activeTab === 'jobs' ? 'Loading print jobs...' : 'Loading queue...'
          "
          :no-data-text="
            activeTab === 'jobs' ? 'No print jobs found' : 'No jobs in queue'
          "
        ...
        >
          ...
          <template #item.status="{ item }">
            <v-chip
              ...
              "
              :icon="
                activeTab === 'jobs' ? getStatusIcon(item.status) : undefined
              "
              size="small"
              variant="elevated"
            >
          ...
          >
            <div
              v-if="item.statistics?.actualPrintTimeSeconds"
              class="text-body-2"
            >
              <v-chip
                :color="
                  getDurationColor(item.statistics.actualPrintTimeSeconds)
                "
                size="small"
                variant="tonal"
              >
          ...
          <template #item.fileName="{ item }">
            <div class="d-flex align-center">
              ...
              <div>
                ...
                <div
                  v-if="
                    activeTab === 'jobs' && item.metadata?.gcodePrintTimeSeconds
                  "
                  class="text-caption text-medium-emphasis"
                >
                ...
                <div
                  v-if="activeTab === 'queue' && item.estimatedTimeSeconds"
                  class="text-caption text-medium-emphasis"
                >
          ...
          <template #item.filament="{ item }">
                ...
                <template v-if="Array.isArray(item.metadata.filamentUsedGrams)">
                  ...
                  >
                    {{ val != null ? Math.round(val) : '-' }}g<span
                      v-if="
                        Number(idx) < item.metadata.filamentUsedGrams.length - 1
                      "
                      >,
                    </span>
                  </span>
            ...
            <div
              v-else-if="
                activeTab === 'queue' &&
                item.filamentGrams !== undefined &&
                item.filamentGrams !== null
              "
              class="text-body-2"
            >
              <v-chip
              ...
              >
                ...
                <template v-if="Array.isArray(item.filamentGrams)">
                  <span
                    v-for="(val, idx) in item.filamentGrams"
                    :key="idx"
                  >
                    {{ val != null ? val.toFixed(1) : '-' }}g<span
                      v-if="Number(idx) < item.filamentGrams.length - 1"
                      >,
                    </span>
                  </span>
                </template>
    ...
    <v-dialog
      v-model="showCompleteConfirmDialog"
      max-width="500"
    >
      <v-card>
        ...
        <v-card-text class="pt-4">
          ...
          <p class="text-body-2 text-medium-emphasis">
            This will set the job status to COMPLETED and update the end time to
            now.
          </p>
        </v-card-text>
    ...
    <v-dialog
      v-model="showFailedConfirmDialog"
      max-width="500"
    >
      <v-card>
        ...
        <v-card-text class="pt-4">
          ...
          <p class="text-body-2 text-medium-emphasis">
            This will set the job status to FAILED and update the end time to
            now.
          </p>
        </v-card-text>
    ...
    <v-dialog
      v-model="showCancelledConfirmDialog"
      max-width="500"
    >
      <v-card>
        ...
        <v-card-text class="pt-4">
          ...
          <p class="text-body-2 text-medium-emphasis">
            This will set the job status to CANCELLED and update the end time to
            now.
          </p>
        </v-card-text>
    ...
    <v-dialog
      v-model="showUnknownConfirmDialog"
      max-width="500"
    >
      <v-card>
    ...
    <v-dialog
      v-model="showSubmitToPrinterDialog"
      max-width="500"
    >
      <v-card>
        <v-card-title class="d-flex align-center bg-success text-on-success">
...
import {
  PrintJobService,
  type PrintJobDto,
  type PrintJobSearchPagedParams
} from '@/backend/print-job.service'
...
const filteredPrintJobs = computed(() => {
  ...
  if (selectedMaterialTypes.value.length > 0) {
    filtered = filtered.filter((job) => {
      return (
        job.metadata?.filamentType &&
        selectedMaterialTypes.value.includes(job.metadata.filamentType)
      )
    })
  }
  ...
  if (selectedPrinterModels.value.length > 0) {
    filtered = filtered.filter((job) => {
      return (
        job.metadata?.printerModel &&
        selectedPrinterModels.value.includes(job.metadata.printerModel)
      )
    })
  }
...
const currentItemsPerPage = computed({
  get: () =>
    activeTab.value === 'jobs' ? itemsPerPage.value : queuePageSize.value,
  set: (val) => {
...
const currentPageNumber = computed({
  get: () =>
    activeTab.value === 'jobs' ? currentPage.value : queueCurrentPage.value,
  set: (val) => {
    if (activeTab.value === 'jobs') {
...
const loadPrintJobs = async () => {
  console.debug(
    '[LoadJobs] loadPrintJobs called, loading state:',
    loading.value
  )
...
const removeFromQueue = async (printerId: number, jobId: number) => {
  try {
    ...
    error(
      'Remove Failed',
      err?.response?.data?.message || 'Failed to remove job from queue'
    )
  }
}
...
const confirmMarkAsCompleted = async () => {
  ...
  try {
    const updatedJob = await PrintJobService.setJobCompleted(
      jobToComplete.value.id
    )

    // Update the job in the list
    const index = printJobs.value.findIndex(
      (j) => j.id === jobToComplete.value!.id
    )
    if (index !== -1) {
      printJobs.value[index] = updatedJob
    }
...
const confirmMarkAsFailed = async () => {
  ...
  try {
    ...
    error(
      ...
      err?.response?.data?.message ||
        err?.message ||
        'Failed to mark job as failed. Please try again.'
    )
  } finally {
    failing.value = false
  }
...
const confirmMarkAsCancelled = async () => {
  ...
  try {
    const updatedJob = await PrintJobService.setJobCancelled(
      jobToCancel.value.id
    )

    // Update the job in the list
    const index = printJobs.value.findIndex(
      (j) => j.id === jobToCancel.value!.id
    )
    if (index !== -1) {
      printJobs.value[index] = updatedJob
    }
    ...
    error(
      ...
      err?.response?.data?.message ||
        err?.message ||
        'Failed to mark job as cancelled. Please try again.'
    )
  } finally {
...
const confirmMarkAsUnknown = async () => {
  ...
  try {
    const updatedJob = await PrintJobService.setJobUnknown(
      jobToSetUnknown.value.id
    )

    // Update the job in the list
    const index = printJobs.value.findIndex(
      (j) => j.id === jobToSetUnknown.value!.id
    )
    if (index !== -1) {
      printJobs.value[index] = updatedJob
    }
    ...
    error(
      ...
      err?.response?.data?.message ||
        err?.message ||
        'Failed to mark job as unknown. Please try again.'
    )
  } finally {
...
const confirmDeleteJob = async () => {
  ...
  deleting.value = true
  try {
    const response = await PrintJobService.deleteJob(
      jobToDelete.value.id,
      deleteFileWithJob.value
    )

    // Remove the job from the list
    const index = printJobs.value.findIndex(
      (j) => j.id === jobToDelete.value!.id
    )
    if (index !== -1) {
    ...
    if (response?.fileDeleted) {
      ...
      message += ` File kept (used by ${
        response.remainingReferences
      } other job${response.remainingReferences > 1 ? 's' : ''}).`
    } else if (deleteFileWithJob.value && !response?.fileDeleted) {
      message += ' File was kept (no other jobs reference it).'
    }

    info('Job Deleted', message, 4000)
...
const confirmAddToQueue = async () => {
  ...
  try {
    ...
    error(
      ...
      err?.response?.data?.message ||
        err?.message ||
        'Failed to add job to queue. Please try again.'
    )
  } finally {
...
const confirmSubmitToPrinter = async () => {
  ...
  try {
    ...
    await PrintQueueService.submitToPrinter(
      jobToSubmit.value.jobId,
      selectedPrinterForSubmit.value
    )
    ...
    error(
      ...
      err?.response?.data?.message ||
        err?.message ||
        'Failed to submit job to printer. Please try again.'
    )
  } finally {
