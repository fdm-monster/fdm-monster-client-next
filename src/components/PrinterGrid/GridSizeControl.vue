<template>
  <div
    class="d-flex align-center ga-1 pa-2 rounded"
...
async function incrementGridRows() {
  if (
    ...
    settingsStore.frontendSettings.gridRows >= 16
  )
    return
...
async function decrementGridRows() {
  if (
    !settingsStore.frontendSettings ||
    settingsStore.frontendSettings.gridRows <= 1
  )
    return
  await settingsStore.updateFrontendSettings({
    ...settingsStore.frontendSettings,
...
async function incrementGridCols() {
  if (
    !settingsStore.frontendSettings ||
    settingsStore.frontendSettings.gridCols >= 12
  )
    return
  await settingsStore.updateFrontendSettings({
    ...settingsStore.frontendSettings,
...
async function decrementGridCols() {
  if (
    !settingsStore.frontendSettings ||
    settingsStore.frontendSettings.gridCols <= 1
  )
    return
  await settingsStore.updateFrontendSettings({
    ...settingsStore.frontendSettings,
