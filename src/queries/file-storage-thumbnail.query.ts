...
export const useFileStorageThumbnailQuery = (
  ...
  thumbnails:
    | ComputedRef<ThumbnailInfo[] | undefined>
    | Ref<ThumbnailInfo[] | undefined>,
  predeterminedThumbnailIndex?:
  ...
  const thumbnailIndexToFetch = computed(() => {
    if (
      predeterminedThumbnailIndex?.value !== undefined &&
      predeterminedThumbnailIndex.value !== null
    ) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        return await FileStorageService.getThumbnailBase64(
          fileStorageId.value,
          thumbnailIndexToFetch.value
        )
      } catch (err) {
        console.debug(
          `Failed to load thumbnail for file ${fileStorageId.value}:`,
          err
        )
        return null
      }
    ...
    enabled:
      !!fileStorageId.value &&
      thumbnailIndexToFetch.value !== null &&
      thumbnailIndexToFetch.value !== undefined &&
      enabled !== false,
